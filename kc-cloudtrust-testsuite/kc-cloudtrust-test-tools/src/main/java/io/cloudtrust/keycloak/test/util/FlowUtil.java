...
public class FlowUtil {
    ...
    private Pattern identifierAtEndPattern = Pattern.compile("/([^/]+)$");
    ...
    public FlowUtil createStandardBrowserFlow(String alias, boolean addUserConfiguredAuth, Consumer<FlowUtil> flowConsumer) {
    	return createFlow(alias, f -> f
    			...
    			.addSubFlowExecution(Requirement.ALTERNATIVE, subflow -> subflow
    					...
    					.addSubFlowExecution(Requirement.CONDITIONAL, condflow -> {
    ...
    public FlowUtil addSubFlowExecution(Requirement requirement, Consumer<FlowUtil> flowInitializer) {
        return addSubFlowExecution("sf" + rand.nextInt(), BASIC_FLOW, requirement, flowInitializer);
    }
    ...
    public FlowUtil addSubFlowExecution(String alias, String providerId, Requirement requirement, int priority, Consumer<FlowUtil> flowInitializer) {
        ...
        return addSubFlowExecution(flowRep, requirement, priority, flowInitializer);
    }
    ...
    public FlowUtil addSubFlowExecution(AuthenticationFlowRepresentation flowRep, Requirement requirement, Consumer<FlowUtil> flowInitializer) {
        return addSubFlowExecution(flowRep, requirement, maxPriority + 10, flowInitializer);
    }

    public FlowUtil addSubFlowExecution(AuthenticationFlowRepresentation flowRep, Requirement requirement, int priority, Consumer<FlowUtil> flowInitializer) {
        maxPriority = Math.max(maxPriority, priority);
